...
function rawDataToProtocol(jsonString) {
  ...
  // 报文帧
  const frameHexString = jsonData.data.slice(5)
  // 标识符
...
function protocolToRawData(jsonString) {
  ...
  if (functionCode === 0x04 || functionCode === 0x03) {
    ...
    return `HD45:${dataHexStr}${toHexString(calculateCRC16(dataFrame))}`
    ...
    const dataFrame = new Uint8Array(buffer)
    return `HD45:${frameHexStr}${toHexString(calculateCRC16(dataFrame))}`
  } else {
